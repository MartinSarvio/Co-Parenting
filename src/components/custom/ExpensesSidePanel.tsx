...
const EXPENSES_TABS = [
  { id: 'balance', label: 'Balance', icon: ArrowRightLeft },
  { id: 'send-penge', label: 'Send/modtag penge', icon: CreditCard },
  { id: 'expenses', label: 'Udgifter', icon: Receipt },
  { id: 'budget', label: 'Budget', icon: Target },
  { id: 'gaveoenskeliste', label: 'Gave/ønskeliste', icon: Gift },
  { id: 'analyse', label: 'Analyse', icon: BarChart3 },
] as const;

export const EXPENSES_TAB_IDS = EXPENSES_TABS.map((t) => t.id);

export function ExpensesSidePanel() {
  const { activeTab, setActiveTab, sideMenuOpen, setSideMenuOpen, sideMenuContext } = useAppStore();

  return createPortal(
    <AnimatePresence>
      {sideMenuOpen && sideMenuContext === 'expenses' && (
        <>
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            <div className="flex items-center justify-between px-5 py-3 border-b border-border">
              <h2 className="text-[17px] font-bold text-foreground">Økonomi</h2>
              <button
                onClick={() => setSideMenuOpen(false)}
