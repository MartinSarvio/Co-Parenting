    ...
    <BottomSheet open={open} onOpenChange={onOpenChange} title={`${item.emoji} ${item.label}`}>
      <div className="space-y-3">
        <div className="space-y-1">
          <Label className="text-[12px] font-semibold uppercase tracking-[0.05em] text-[#78766d]">Tidspunkt</Label>
          <Input
            type="time"
            value={time}
        ...
        <div className="space-y-1">
          <Label className="text-[12px] font-semibold uppercase tracking-[0.05em] text-[#78766d]">Note (valgfrit)</Label>
          <Textarea
            value={note}
            onChange={e => setNote(e.target.value)}
            placeholder="f.eks. Spiste godt, drak 200ml..."
            className="min-h-[60px] resize-none rounded-[4px] border-[#d8d7cf] bg-white text-sm"
            maxLength={500}
          />
        </div>
