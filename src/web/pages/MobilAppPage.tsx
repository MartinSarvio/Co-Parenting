...
const features = [
  { icon: Bell, title: 'Push-notifikationer', desc: 'Få besked med det samme når der sker noget nyt — nye beskeder, opgaver eller ændringer i samværsplanen.', color: '#f58a2d' },
  { icon: WifiOff, title: 'Offline-adgang', desc: 'Se samværsplan, opgaver og beskeder selv uden internet. Data synkroniseres automatisk når du er online igen.', color: '#3b82f6' },
  { icon: Fingerprint, title: 'Face ID & Touch ID', desc: 'Hurtig og sikker adgang med biometrisk login. Dine data er altid beskyttet.', color: '#8b5cf6' },
  { icon: LayoutGrid, title: 'Widgets', desc: 'Se dagens samvær og kommende opgaver direkte på din startskærm uden at åbne appen.', color: '#10b981' },
  { icon: Moon, title: 'Mørk tilstand', desc: 'Behageligt for øjnene om aftenen. Skifter automatisk med din telefons indstillinger.', color: '#6366f1' },
  { icon: Zap, title: 'Hurtig synkronisering', desc: 'Ændringer synkroniseres i realtid mellem alle familiemedlemmer. Altid opdateret.', color: '#f43f5e' },
];
...
const details = [
    ...
    visual: (
      <div className="relative">
        <div className="w-[240px] h-[480px] rounded-[2.5rem] border-[8px] border-[#2f2f2f] bg-[#1a1a1a] overflow-hidden shadow-2xl">
          <div className="w-full h-full bg-gradient-to-b from-[#fff7ed] to-[#fff0de] flex items-center justify-center">
            <div className="text-center px-6">
              <Calendar size={48} className="mx-auto mb-4" style={{ color: '#f58a2d' }} />
      ...
      <div className="space-y-3 max-w-[280px]">
        {[
          ...
          <div key={i} className={`flex ${m.align === 'right' ? 'justify-end' : 'justify-start'}`}>
            <div className={`px-4 py-2.5 rounded-2xl text-[13px] max-w-[200px] ${m.align === 'right' ? 'bg-[#8b5cf6] text-white rounded-br-md' : 'bg-white/80 text-[#2f2f2f] border border-white/40 rounded-bl-md'}`}>
              {m.msg}
            </div>
          </div>
        ))}
  ...
  },
  {
    ...
    visual: (
      <div className="relative">
        <div className="w-[240px] h-[480px] rounded-[2.5rem] border-[8px] border-[#2f2f2f] bg-[#1a1a1a] overflow-hidden shadow-2xl">
          <div className="w-full h-full bg-gradient-to-b from-[#ecfdf5] to-[#d1fae5] flex items-center justify-center">
            <div className="text-center px-6">
              <Wallet size={48} className="mx-auto mb-4" style={{ color: '#10b981' }} />
      ...
      heroVisual={
        <div className="relative">
          ...
          <div className="absolute -top-4 -left-4 w-14 h-14 rounded-2xl bg-white shadow-xl flex items-center justify-center">
            <Download size={28} style={{ color: '#f58a2d' }} />
          </div>
        </div>
      }
    />
