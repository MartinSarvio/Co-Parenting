...
const features = [
  { icon: CalendarDays, title: 'Fælles begivenheder', desc: 'Tilføj begivenheder begge forældre kan se. Forældremøder, fødselsdage, aktiviteter.', color: '#3b82f6' },
  { icon: Gift, title: 'Vigtige datoer', desc: 'Glem aldrig fødselsdage, milepæle og jubilæer. Automatiske påmindelser.', color: '#ef4444' },
  { icon: Bell, title: 'Påmindelser', desc: 'Sæt påmindelser for begivenheder. Vælg hvornår du vil have besked — en dag, en uge eller en time før.', color: '#f58a2d' },
  { icon: RefreshCw, title: 'Synk med telefon', desc: 'Synkronisér med din iPhones kalender. Se Huska-begivenheder sammen med dine andre aftaler.', color: '#8b5cf6' },
  { icon: Stethoscope, title: 'Lægebesøg & kontroller', desc: 'Hold styr på børnenes sundhedsaftaler. Begge forældre har adgang til alle informationer.', color: '#10b981' },
  { icon: CalendarHeart, title: 'Skoledage & ferier', desc: 'Importér skolens ferieplan og planlæg herefter. Aldrig tvivl om skolefridage.', color: '#06b6d4' },
];
      ...
      <div className="max-w-[260px]">
        <div className="p-4 rounded-2xl bg-white/70 border border-white/40">
          ...
          <div className="grid grid-cols-7 gap-1 text-center text-[11px]">
            ...
            {Array.from({ length: 31 }, (_, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`py-1.5 rounded-lg text-[12px] ${hasEvent ? 'bg-[#3b82f6]/15 text-[#3b82f6] font-bold' : 'text-[#5f5d56]'}`}
                >
                  {i + 1}
                </div>
              );
          ...
          <div className="mt-3 space-y-1.5">
            {[
              { date: '5. mar', event: 'Forældremøde', color: '#3b82f6' },
              { date: '18. mar', event: 'Emmas fødselsdag', color: '#ef4444' },
              { date: '23. mar', event: 'Tandlæge', color: '#10b981' },
            ].map((e, i) => (
