...
export function Aarskalender() {
  ...
  const p1Color = parent1?.id === currentUser?.id ? 'bg-primary/20' : 'bg-[#f58a2d]/20';
  const p2Color = parent2?.id === currentUser?.id ? 'bg-primary/20' : 'bg-[#f58a2d]/20';
  ...
  return (
    <div className="space-y-2 py-1">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-[1.35rem] font-bold tracking-[-0.02em] text-foreground">Årskalender</h1>
        <div className="flex items-center gap-1">
          ...
          <span className="w-12 text-center text-[14px] font-bold text-foreground">{year}</span>
          <button
            onClick={() => setYear(y => y + 1)}
            className="flex h-8 w-8 items-center justify-center text-muted-foreground transition-colors"
          >
      ...
      {/* Legend */}
      {parent1 && parent2 && (
        <div className="flex items-center gap-4 rounded-[8px] border border-border bg-card px-4 py-3 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
          <div className="flex items-center gap-2">
            <div className={cn('h-3.5 w-3.5 rounded-sm', p1Color)} />
            <span className="text-[12px] font-medium text-foreground">{parent1.name}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className={cn('h-3.5 w-3.5 rounded-sm', p2Color)} />
            <span className="text-[12px] font-medium text-foreground">{parent2.name}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className={cn('h-3.5 w-3.5 rounded-sm', holidayColor)} />
            <span className="text-[12px] font-medium text-foreground">Ferie</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="h-3.5 w-3.5 rounded-sm bg-muted ring-1 ring-ring" />
            <span className="text-[12px] font-medium text-foreground">Event</span>
          </div>
        </div>
      )}
      ...
      {/* Child info */}
      {currentChild && (
        <div className="flex items-center gap-2 rounded-[8px] bg-card px-4 py-2.5 border border-border">
      ...
      {!custodyPlan && (
        <div className="rounded-[8px] border border-dashed border-border bg-card px-4 py-6 text-center">
          <p className="text-[13px] text-muted-foreground">Ingen samværsplan oprettet endnu. Opret en plan under Samvær for at se fordelingen her.</p>
        </div>
      )}
        ...
        {months.map(month => {
          ...
          return (
            <div
              ...
              className="rounded-[8px] border border-border bg-card px-3 py-3 shadow-[0_2px_8px_rgba(0,0,0,0.04)] cursor-pointer hover:border-[#f58a2d]/40 transition-colors"
              onClick={() => setSelectedMonth(month)}
            >
              <p className="mb-2 text-center text-[12px] font-bold tracking-[-0.01em] text-foreground capitalize">
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        dayColor || 'bg-muted',
                        todayDay && 'ring-1 ring-ring ring-offset-0',
                      )}
                    >
                      <span className={cn(dayColor ? 'text-foreground' : 'text-muted-foreground')}>
                        {format(day, 'd')}
                      </span>
                      {hasEv && (
      ...
      {custodyPlan?.holidays && custodyPlan.holidays.length > 0 && (
        <div className="space-y-2">
          <h2 className="text-[0.95rem] font-semibold tracking-[-0.01em] text-foreground">Ferieaftaler</h2>
          {custodyPlan.holidays.map(h => {
            const p = users.find(u => u.id === h.parentId);
            return (
              <div key={h.id} className="flex items-center justify-between rounded-[8px] border border-border bg-card px-4 py-3 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
                <div>
      ...
      <AnimatePresence>
        {selectedMonth && (() => {
          ...
          return (
            ...
            >
              {/* Header */}
              <div className="shrink-0 flex items-center justify-between px-4 pt-[env(safe-area-inset-top,16px)] pb-3 bg-card border-b border-border">
                <button
                  onClick={() => setSelectedMonth(null)}
                  className="flex h-9 w-9 items-center justify-center rounded-[8px] bg-background text-foreground active:scale-[0.92] transition-transform"
                >
                  <ArrowLeft className="h-[18px] w-[18px]" />
                </button>
                <h1 className="text-[17px] font-bold text-foreground capitalize">
              ...
              <div className="flex-1 overflow-y-auto">
                <div className="px-4 pt-4 pb-8 max-w-[430px] mx-auto">
                  ...
                  <div className="mb-2 grid grid-cols-7 gap-1">
                    {['M', 'T', 'O', 'T', 'F', 'L', 'S'].map((d, i) => (
                      <div key={i} className="text-center text-[11px] font-semibold text-muted-foreground">{d}</div>
                    ))}
                  </div>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            dayColor || 'bg-muted',
                            todayDay && 'ring-1 ring-ring ring-offset-1',
                          )}
                        >
                          <span className={cn(dayColor ? 'text-foreground' : 'text-muted-foreground')}>
                            {format(day, 'd')}
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="mt-4 space-y-2">
                        <h3 className="text-[13px] font-semibold text-foreground">Begivenheder denne måned</h3>
                        {monthEvents.map(ev => (
                          <div key={ev.id} className="flex items-center gap-3 rounded-[8px] border border-border bg-card px-3 py-2 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
        ...
        {quickAddDate && (
          ...
          >
            ...
            <div className="shrink-0 flex items-center justify-between px-4 pt-[env(safe-area-inset-top,16px)] pb-3 bg-card border-b border-border">
              ...
              <h1 className="text-[17px] font-bold text-foreground">Ny begivenhed</h1>
              <div className="w-9" />
            </div>
              ...
              <div className="px-4 pt-4 pb-8 max-w-[430px] mx-auto space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 overflow-hidden">
                  <div className="space-y-1.5 min-w-0 overflow-hidden">
                    <label className="text-[12px] font-semibold text-muted-foreground">Starttid</label>
                    <input
                      type="time"
                  ...
                  <div className="space-y-1.5 min-w-0 overflow-hidden">
                    <label className="text-[12px] font-semibold text-muted-foreground">Sluttid</label>
                    <input
                      type="time"
                ...
                {/* Location */}
                <div className="space-y-1.5">
                  <label className="text-[12px] font-semibold text-muted-foreground">Lokation</label>
                  <input
                    type="text"
                ...
                <button
                  ...
                  className="w-full rounded-[8px] bg-primary py-4 text-[15px] font-bold text-white active:scale-[0.98] transition-transform disabled:opacity-40"
                >
                  Gem begivenhed
                </button>
              </div>
