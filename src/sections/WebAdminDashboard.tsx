  ...
  return (
    <div className="min-h-screen bg-card">
      ...
      <header className="bg-card border-b border-border px-6 py-3 flex items-center gap-4 sticky top-0 z-10">
        ...
        <h1 className="text-[16px] font-bold text-foreground">Huska <span className="text-[12px] font-medium text-muted-foreground ml-1">Admin</span></h1>
        <div className="flex-1" />
        <span className="text-[12px] text-muted-foreground">{user.email}</span>
        <button onClick={handleLogout} className="p-2 rounded-lg hover:bg-muted text-muted-foreground" title="Log ud">
...
function LoginForm() {
  ...
  return (
    <div className="min-h-screen bg-card flex items-center justify-center">
      <form onSubmit={handleLogin} className="bg-card rounded-xl shadow-sm border border-border p-8 w-full max-w-[400px] space-y-4">
        <div className="text-center mb-6">
          <img src="/huska-logo.svg" alt="Huska" className="h-12 w-12 rounded-2xl mx-auto mb-3" />
          <h1 className="text-[18px] font-bold text-foreground">Huska <span className="text-muted-foreground font-medium">Admin</span></h1>
          <p className="text-[13px] text-muted-foreground mt-1">Log ind for at administrere indhold</p>
        </div>
        <div>
          <label className="text-[12px] font-semibold text-muted-foreground block mb-1">E-mail</label>
          <input
            type="email"
            value={email}
            onChange={e => setEmail(e.target.value)}
            className="w-full h-10 rounded-lg border border-border px-3 text-[13px]"
            required
          />
        </div>
        <div>
          <label className="text-[12px] font-semibold text-muted-foreground block mb-1">Adgangskode</label>
          <input
            type="password"
            value={password}
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full h-10 rounded-lg bg-primary text-white font-semibold text-[14px] hover:bg-primary disabled:opacity-50"
        >
...
function TilbudsaviserTab() {
  ...
  const BatchTable = ({ title, items }: { title: string; items: ImportBatch[] }) => {
    ...
    return (
      <div className="mb-6">
        <h3 className="text-[13px] font-semibold text-muted-foreground mb-2">{title} ({items.length})</h3>
        <div className="bg-card rounded-lg border border-border overflow-hidden">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-border bg-card">
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Butik</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Fil</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Periode</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground text-center">Produkter</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground text-center">Status</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground"></th>
              </tr>
            </thead>
            <tbody>
              {items.map((b, i) => (
                <tr key={b.id} className={i % 2 === 0 ? 'bg-card' : 'bg-card'}>
                  <td className="px-4 py-2.5 text-[13px] font-semibold text-foreground">{b.store ?? '—'}</td>
                  <td className="px-4 py-2.5 text-[12px] text-muted-foreground truncate max-w-[200px]">{b.file_name}</td>
                  ...
                  <td className="px-4 py-2.5 text-center text-[13px] font-semibold text-foreground">{b.total_products || 0}</td>
                  <td className="px-4 py-2.5 text-center">{statusBadge(b.status)}</td>
                  <td className="px-4 py-2.5">
                    <div className="flex gap-1 justify-end">
                      {b.total_products > 0 && (
                        <button onClick={() => loadPreview(b)} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground" title="Se produkter">
                          <Eye className="h-3.5 w-3.5" />
                        </button>
                      )}
                      {b.total_products > 0 && b.file_url && (
                        <button onClick={() => setCropBatch(b)} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground" title="Tilknyt billeder">
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-[18px] font-bold text-foreground">Tilbudsaviser</h2>
        <button
          onClick={() => setShowUpload(true)}
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white text-[13px] font-semibold hover:bg-primary"
        >
      ...
      <div className="mb-6">
        <h3 className="text-[13px] font-semibold text-muted-foreground mb-2">Aviser i appen ({FLYERS.length})</h3>
        <div className="bg-card rounded-lg border border-border overflow-hidden">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-border bg-card">
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Cover</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Butik</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Periode</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground text-center">Sider</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground text-center">Status</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground"></th>
              </tr>
            </thead>
            <tbody>
              {FLYERS.map((f, i) => {
                ...
                return (
                  <tr key={f.id} className={i % 2 === 0 ? 'bg-card' : 'bg-card'}>
                    ...
                    <td className="px-4 py-2.5 text-[12px] text-muted-foreground">{f.validFrom} – {f.validUntil}</td>
                    <td className="px-4 py-2.5 text-center text-[13px] font-semibold text-foreground">{f.pages}</td>
                    ...
                    <td className="px-4 py-2.5">
                      <a
                        ...
                        className="p-1.5 rounded-md hover:bg-muted text-muted-foreground inline-flex"
        ...
        <>
          ...
          {batches.length === 0 && (
            <div className="text-center py-10">
              ...
              <p className="text-[13px] text-muted-foreground">Ingen uploadede tilbudsaviser endnu — brug knappen ovenfor</p>
            </div>
          )}
        </>
      )}
...
function UploadModal({ allBatches, onClose, onSaved }: { allBatches: ImportBatch[]; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center" onClick={onClose}>
      <div className="bg-card rounded-xl shadow-xl w-full max-w-[500px] p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[16px] font-bold text-foreground">Upload tilbudsavis</h3>
        ...
        <div className="space-y-4">
          {/* Store */}
          <div>
            <label className="text-[12px] font-semibold text-muted-foreground block mb-1">Butikskæde *</label>
            <select
              value={store}
          ...
          {/* PDF */}
          <div>
            <label className="text-[12px] font-semibold text-muted-foreground block mb-1">PDF-fil *</label>
            ...
            <div
              ...
              className="rounded-lg border-2 border-dashed border-border bg-card hover:border-border p-6 text-center cursor-pointer transition-colors"
            >
              {pdfFile ? (
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-[13px] font-semibold text-foreground">Klik for at vælge PDF</p>
                  <p className="text-[11px] text-muted-foreground">Max 70 MB</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[12px] font-semibold text-muted-foreground block mb-1">Gyldig fra</label>
              <input type="date" value={validFrom} onChange={e => setValidFrom(e.target.value)} className="w-full h-10 rounded-lg border border-border px-3 text-[13px]" />
            </div>
            <div>
              <label className="text-[12px] font-semibold text-muted-foreground block mb-1">Gyldig til</label>
              <input type="date" value={validUntil} onChange={e => setValidUntil(e.target.value)} className="w-full h-10 rounded-lg border border-border px-3 text-[13px]" />
            </div>
          </div>
        ...
        <div className="flex gap-2 mt-6">
          <button onClick={onClose} className="flex-1 h-10 rounded-lg border border-border text-[13px] font-medium text-muted-foreground hover:bg-muted">
            Annuller
          </button>
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 h-10 rounded-lg bg-primary text-white text-[13px] font-semibold hover:bg-primary disabled:opacity-50"
          >
...
function PreviewModal({ batch, items, onClose }: { batch: ImportBatch; items: ImportItem[]; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center" onClick={onClose}>
      <div className="bg-card rounded-xl shadow-xl w-full max-w-[800px] max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div>
            <h3 className="text-[16px] font-bold text-foreground">{batch.store} — {batch.total_products} produkter</h3>
            <p className="text-[12px] text-muted-foreground">{batch.file_name}</p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground">
            <X className="h-4 w-4" />
          </button>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b)).map(([cat, catItems]) => (
            <div key={cat} className="mb-4">
              <h4 className="text-[12px] font-semibold text-muted-foreground uppercase tracking-wide mb-2">{cat} ({catItems.length})</h4>
              <div className="space-y-1">
                {catItems.map(item => (
                  <div key={item.id} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-card">
                    {item.image_url ? (
                    ...
                    {item.original_price && (
                      <span className="text-[12px] text-muted-foreground line-through">{item.original_price} kr</span>
                    )}
                    <span className="text-[14px] font-bold text-foreground">{item.price} kr</span>
...
function CropModal({ batch, onClose }: { batch: ImportBatch; onClose: () => void; onUpdated?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex" onClick={onClose}>
      <div className="bg-card w-full h-full flex" onClick={e => e.stopPropagation()}>
        {/* Left: Product list */}
        <div className="w-[300px] border-r border-border flex flex-col">
          <div className="px-4 py-3 border-b border-border flex items-center justify-between">
            <h3 className="text-[14px] font-bold text-foreground">Produkter</h3>
            <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground">
              <X className="h-4 w-4" />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto">
            {items.map(item => (
              <button
                ...
                className={`w-full text-left px-4 py-2.5 border-b border-border hover:bg-card flex items-center gap-2 ${selectedItem?.id === item.id ? 'bg-gray-100' : ''}`}
              >
                {item.image_url ? (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-foreground font-semibold">{item.price} kr</p>
                </div>
        ...
        <div className="flex-1 flex flex-col bg-muted">
          ...
          <div className="bg-card border-b border-border px-4 py-2 flex items-center gap-3">
            <span className="text-[12px] text-muted-foreground">Side:</span>
            <div className="flex gap-1 flex-wrap">
              {pages.map((_, i) => (
                <button
                  key={i}
                  onClick={() => { setSelectedPage(i); setCropStart(null); setCropEnd(null); }}
                  className={`w-7 h-7 rounded text-[11px] font-medium ${selectedPage === i ? 'bg-primary text-white' : 'bg-muted text-muted-foreground hover:bg-muted'}`}
                >
                  {i + 1}
                </button>
              ))}
          ...
          <div className="flex-1 overflow-auto p-4 flex items-start justify-center">
            {loadingPdf ? (
              <div className="flex flex-col items-center gap-3 py-20">
                <RefreshCw className="h-6 w-6 animate-spin text-muted-foreground" />
                <p className="text-[13px] text-muted-foreground">Indlæser PDF...</p>
              </div>
            ) : pages.length === 0 ? (
              <p className="text-[13px] text-muted-foreground py-20">Ingen sider fundet</p>
            ) : (
...
function AffiliateTab() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-[18px] font-bold text-foreground">Affiliate-links</h2>
        <button
      ...
      <div className="mb-6">
        <h3 className="text-[13px] font-semibold text-muted-foreground mb-2">Standard sponsors i appen ({DEFAULT_SPONSORS.length})</h3>
        <div className="bg-card rounded-lg border border-border overflow-hidden">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-border bg-card">
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Banner</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Navn</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Butik</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Kategori</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Type</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground"></th>
              </tr>
            </thead>
            <tbody>
              {DEFAULT_SPONSORS.map((s, i) => (
                <tr key={s.id} className={i % 2 === 0 ? 'bg-card' : 'bg-card'}>
                  <td className="px-4 py-2">
                    {s.banner_image ? (
                  ...
                  <td className="px-4 py-2">
                    ...
                    <p className="text-[10px] text-muted-foreground truncate max-w-[200px]">{s.url}</p>
                  </td>
                  <td className="px-4 py-2 text-[12px] text-muted-foreground">{s.store}</td>
                  <td className="px-4 py-2 text-[12px] text-muted-foreground">{s.category}</td>
                  <td className="px-4 py-2">
      ...
      <h3 className="text-[13px] font-semibold text-muted-foreground mb-2">Database affiliate-links</h3>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full h-10 rounded-lg border border-border pl-10 pr-3 text-[13px]"
        />
      </div>
      ...
      <div className="bg-card rounded-lg border border-border overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-[14px] font-semibold text-foreground">Ingen affiliate-links</p>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-border bg-card">
                ...
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Butik</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground">Kategori</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground text-center">Aktiv</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground text-right">Klik</th>
                <th className="px-4 py-2 text-[11px] font-semibold text-muted-foreground"></th>
              </tr>
            </thead>
            <tbody>
              {filteredLinks.map((link, i) => (
                <tr key={link.id} className={i % 2 === 0 ? 'bg-card' : 'bg-card'}>
                  <td className="px-4 py-2">
                    {link.banner_image ? (
                  ...
                  <td className="px-4 py-2">
                    <p className="text-[13px] font-semibold text-foreground">{link.name}</p>
                    <p className="text-[10px] text-muted-foreground truncate max-w-[200px]">{link.url}</p>
                  </td>
                  <td className="px-4 py-2 text-[12px] text-muted-foreground">{link.store ?? '—'}</td>
                  <td className="px-4 py-2 text-[12px] text-muted-foreground">{link.category ?? '—'}</td>
                  ...
                  <td className="px-4 py-2 text-[13px] font-semibold text-foreground text-right">{link.click_count}</td>
                  <td className="px-4 py-2">
                    <div className="flex gap-1 justify-end">
                      <button onClick={() => setEditLink(link)} className="p-1.5 rounded-md hover:bg-muted text-muted-foreground">
...
function AffiliateFormModal({ link, onClose, onSaved }: { link: AffiliateLink | null; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center" onClick={onClose}>
      <div className="bg-card rounded-xl shadow-xl w-full max-w-[500px] max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[16px] font-bold text-foreground">{link ? 'Rediger' : 'Opret'} affiliate-link</h3>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div>
            <label className="text-[12px] font-semibold text-muted-foreground block mb-1">Navn *</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="F.eks. Bilka Affiliate" className="w-full h-10 rounded-lg border border-border px-3 text-[13px]" />
          </div>
          <div>
            <label className="text-[12px] font-semibold text-muted-foreground block mb-1">Affiliate-URL *</label>
            <input value={url} onChange={e => setUrl(e.target.value)} placeholder="https://..." className="w-full h-10 rounded-lg border border-border px-3 text-[13px]" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[12px] font-semibold text-muted-foreground block mb-1">Butik</label>
              <input list="admin-store-list" value={store} onChange={e => setStore(e.target.value)} placeholder="Vælg butik..." className="w-full h-10 rounded-lg border border-border px-3 text-[13px]" />
              <datalist id="admin-store-list">{STORES.map(s => <option key={s} value={s} />)}</datalist>
            </div>
            <div>
              <label className="text-[12px] font-semibold text-muted-foreground block mb-1">Partner</label>
              <input value={partner} onChange={e => setPartner(e.target.value)} placeholder="Partner-navn" className="w-full h-10 rounded-lg border border-border px-3 text-[13px]" />
            </div>
          </div>
          <div>
            <label className="text-[12px] font-semibold text-muted-foreground block mb-1">Kategori</label>
            <input value={category} onChange={e => setCategory(e.target.value)} placeholder="F.eks. Elektronik, Mejeri" className="w-full h-10 rounded-lg border border-border px-3 text-[13px]" />
          </div>

          {/* Banner */}
          <div>
            <label className="text-[12px] font-semibold text-muted-foreground block mb-1">Banner-billede</label>
            ...
            {bannerImage ? (
              <div className="space-y-2">
                <div className="relative">
                  ...
                  <div className="absolute top-1.5 right-1.5 flex gap-1">
                    <button onClick={() => bannerRef.current?.click()} className="h-7 w-7 rounded-full bg-card/90 flex items-center justify-center shadow-sm">
                      <Pencil className="h-3 w-3 text-muted-foreground" />
                    </button>
                    <button onClick={() => { setBannerImage(''); setBannerFile(null); }} className="h-7 w-7 rounded-full bg-card/90 flex items-center justify-center shadow-sm">
              ...
              <button onClick={() => bannerRef.current?.click()} className="w-full h-20 rounded-lg border-2 border-dashed border-border bg-card hover:border-border flex flex-col items-center justify-center gap-1 transition-colors">
                <ImagePlus className="h-5 w-5 text-muted-foreground" />
                <p className="text-[11px] text-muted-foreground">Upload banner</p>
              </button>
            )}
          </div>

          <div>
            <label className="text-[12px] font-semibold text-muted-foreground block mb-1">Kilde / URL</label>
            <input value={sourceUrl} onChange={e => setSourceUrl(e.target.value)} placeholder="URL til partner-side" className="w-full h-10 rounded-lg border border-border px-3 text-[13px]" />
          </div>

          <div className="flex items-center justify-between rounded-lg border border-border px-4 py-3">
            <span className="text-[13px] font-semibold text-foreground">Aktiv</span>
            <button
              onClick={() => setIsActive(!isActive)}
              className={`w-12 h-7 rounded-full transition-colors ${isActive ? 'bg-[#34C759]' : 'bg-muted'}`}
            >
              <div className={`w-5 h-5 bg-card rounded-full shadow transition-transform mx-1 ${isActive ? 'translate-x-5' : 'translate-x-0'}`} />
            </button>
          </div>
        </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 h-10 rounded-lg border border-border bg-card text-[13px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
          />
        </div>
        <button
          onClick={() => setCreateOpen(true)}
          className="flex items-center gap-2 h-10 px-4 rounded-lg bg-primary text-white text-[13px] font-semibold hover:bg-primary transition-colors"
        >
          <Plus className="h-4 w-4" />
          Opret nyhed
        </button>
      ...
      {loading ? (
        ...
        <div className="rounded-xl bg-card border border-border overflow-hidden">
          {/* Table header */}
          <div className="grid grid-cols-[auto_1fr_120px_100px_80px_100px] items-center gap-3 px-4 py-2.5 bg-card border-b border-border text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">
            <div className="w-10" />
            <div>Titel</div>
            <div>Kilde</div>
            <div>Dato</div>
          ...
          {filtered.map(article => (
            <div
              key={article.id}
              className="grid grid-cols-[auto_1fr_120px_100px_80px_100px] items-center gap-3 px-4 py-3 border-b border-border last:border-b-0 hover:bg-card transition-colors"
            >
              {/* Thumbnail */}
              {article.image ? (
              ...
              <div className="min-w-0">
                ...
                {article.description && (
                  <p className="text-[11px] text-muted-foreground truncate mt-0.5">{article.description}</p>
                )}
              </div>
              ...
              {/* Date */}
              <div className="text-[12px] text-muted-foreground">{article.date ?? '—'}</div>
              ...
              <div className="flex items-center justify-end gap-1">
                ...
                {article.url && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="p-1.5 rounded-md text-muted-foreground hover:text-[#007AFF] hover:bg-blue-50 transition-colors"
                    title="Åbn link"
                  >
                    <ExternalLink className="h-3.5 w-3.5" />
                  </a>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="bg-card rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <h3 className="text-[15px] font-bold text-foreground mb-2">Slet nyhed?</h3>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 h-10 rounded-lg border border-border text-[13px] font-medium text-muted-foreground hover:bg-muted"
              >
                Annuller
              </button>
              <button
    ...
    <div className="max-w-2xl">
      ...
      {mode === 'scrape' && !isEdit && (
        <div className="rounded-xl bg-card border border-border p-4 mb-5">
          <label className="block text-[12px] font-semibold text-muted-foreground mb-1.5">Artikel-URL</label>
          <div className="flex gap-2">
            <input
              value={scrapeUrl}
              onChange={e => setScrapeUrl(e.target.value)}
              placeholder="https://..."
              className="flex-1 h-10 px-3 rounded-lg border border-border text-[13px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
            />
            <button
              onClick={handleScrape}
              disabled={scraping}
              className="flex items-center gap-2 h-10 px-4 rounded-lg bg-primary text-white text-[13px] font-medium hover:bg-primary disabled:opacity-50"
            >
              {scraping ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Globe className="h-3.5 w-3.5" />}
              Hent
            </button>
          </div>
          {scraping && <p className="text-[11px] text-muted-foreground mt-2">Henter artikel-data...</p>}
        </div>
      )}
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-[12px] font-semibold text-muted-foreground mb-1.5">Titel *</label>
          <input
            value={title}
            onChange={e => setTitle(e.target.value)}
            placeholder="Nyhedens titel"
            className="w-full h-10 px-3 rounded-lg border border-border bg-card text-[13px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
          />
        </div>

        <div>
          <label className="block text-[12px] font-semibold text-muted-foreground mb-1.5">Beskrivelse</label>
          <textarea
            value={description}
            onChange={e => setDescription(e.target.value)}
            placeholder="Kort beskrivelse..."
            rows={2}
            className="w-full px-3 py-2 rounded-lg border border-border bg-card text-[13px] text-foreground placeholder:text-muted-foreground resize-y focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
          />
        </div>

        <div>
          <label className="block text-[12px] font-semibold text-muted-foreground mb-1.5">Fuld tekst</label>
          <textarea
            value={fullText}
        ...
        {/* Image preview + URL */}
        <div>
          <label className="block text-[12px] font-semibold text-muted-foreground mb-1.5">Billede-URL</label>
          <input
            value={image}
          ...
          {image && (
            <div className="relative mt-2">
              <img src={image} alt="" className="w-full h-40 object-cover rounded-lg border border-border" />
              <button
                onClick={() => setImage('')}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-[12px] font-semibold text-muted-foreground mb-1.5">Kilde</label>
            <input
              value={source}
          ...
          <div>
            <label className="block text-[12px] font-semibold text-muted-foreground mb-1.5">Dato</label>
            <input
              value={date}
        ...
        <div>
          <label className="block text-[12px] font-semibold text-muted-foreground mb-1.5">Link til artikel</label>
          <input
            value={url}
        ...
        <div className="flex items-center justify-between rounded-xl bg-card border border-border px-4 py-3">
          ...
          <button
            ...
            className={`relative w-11 h-6 rounded-full transition-colors ${isPublished ? 'bg-green-500' : 'bg-muted'}`}
          >
            <div className={`absolute top-0.5 h-5 w-5 rounded-full bg-card shadow transition-transform ${isPublished ? 'translate-x-[22px]' : 'translate-x-0.5'}`} />
          </button>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button
            ...
            className="flex-1 h-10 rounded-lg bg-primary text-white text-[13px] font-semibold hover:bg-primary disabled:opacity-50 flex items-center justify-center gap-2"
          >
